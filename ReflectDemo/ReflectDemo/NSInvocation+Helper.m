...
static void *DCAllocBufferForObjCType(const char *objCType)
{
    void *buffer = NULL;
...
- (void)dc_setArgument:(id)argumentValue atIndex:(NSUInteger)index
{
    ...
    else if ([argumentValue isKindOfClass:[NSValue class]])
    {
        ...
        void *buffer = DCAllocBufferForObjCType(valueArgument.objCType);
...
- (void)DCSetArgumentsFromArray:(NSArray *)argumentArray
{
    ...
    for (id argument in argumentArray) {
        ...
        [self dc_setArgument:argument atIndex:argumentIndex];
        ++i;
    }
}
...
- (id)DCReturnValue {
    ...
    void *buffer = DCAllocBufferForObjCType(objCType);
